...
Tensor nms_1d_cpu(
  ...
  for (int64_t _i = 0; _i < nsegs; _i++) {
    ...
    for (int64_t _j = _i + 1; _j < nsegs; _j++) {
      ...
      if (ovr >= iou_thresh) select[_j] = false;
    }
  }
  return order_t.masked_select(select_t);
}

Tensor nms_1d(Tensor segs, Tensor scores, float iou_thresh) {
  CHECK_CPU_INPUT(segs);
  CHECK_CPU_INPUT(scores);
  return nms_1d_cpu(segs, scores, iou_thresh);

}

Tensor softnms_1d_cpu(
  Tensor segs, 
  Tensor scores, 
  Tensor dets, 
  float iou_thresh,
  ...
  for (int64_t i = 0; i < nsegs; i++) {
    ...
    while (pos < nsegs) {
      ...
      if (method == 0) {
        // vanilla nms
        if (ovr >= iou_thresh) weight = 0;
      } else if (method == 1) {
        // linear
        if (ovr >= iou_thresh) weight = 1 - ovr;
      } else if (method == 2) {
        // gaussian
...
Tensor softnms_1d(Tensor segs, Tensor scores, Tensor dets, float iou_thresh,
                  float sigma, float min_score, int method) {
  // softnms is not implemented on GPU
  CHECK_CPU_INPUT(segs)
  CHECK_CPU_INPUT(scores)
  CHECK_CPU_INPUT(dets)
  return softnms_1d_cpu(segs, scores, dets, iou_thresh, sigma, min_score, method);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def(
    "nms", &nms_1d, "nms (CPU) ",
    py::arg("segs"), py::arg("scores"), py::arg("iou_thresh")
  );
  m.def(
    "softnms", &softnms_1d, "softnms (CPU) ",
    py::arg("segs"), py::arg("scores"), py::arg("dets"), py::arg("iou_thresh"),
